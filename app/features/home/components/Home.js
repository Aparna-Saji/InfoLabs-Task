import axios from 'axios';
import NavigationContainer from '@react-navigation/native';
import { NativeBaseProvider, Box, Text, VStack, HStack, ChevronLeftIcon, Stack, Image, FlatList } from 'native-base'
import Icon from 'react-native-vector-icons/MaterialIcons';
import { View, RefreshControl, Linking, TouchableOpacity } from 'react-native'
import { useCallback, useState } from 'react';
import { useFocusEffect } from '@react-navigation/native';
import { Header } from '../../common/components'
import React, { useEffect } from 'react'
import { } from 'react-native-gesture-handler';

const Home = () => {
  const [productData, setProductData] = useState([]);
  const [loader, setLoader] = useState(false);
  const [facebook,setFacebook]=useState( '');
  const [website,setWebsite]=useState('');
  const [email,setEmail]=useState('');
  const [location,setLocation]=useState('');


  useFocusEffect(
    useCallback(() => {
      getAllProduct()
      return () => {

      }
    }, [])
  );

  // useEffect(() => {
  //   console.log("productData  ", productData);
  //   console.log("productData img  ", productData);
  // }, [productData])



  const getAllProduct = async () => {
    axios.post('https://www.infonlive.com/api/business-listings', {
      category_id: 1,
      subcategory_id: 85,
      city_unique_code: 1100019,
      district_id: 11,
      latitude: 23.0225,
      longitude: 72.5714,
    })
      .then((res) => {
        console.log("RESPONSE=======", res);
        setProductData(res?.data?.data)
      })
      .catch((e) => {
        console.log("ERR", e);
      })
  }






  return (
    <NativeBaseProvider>
      <Box flex={1} bg='#33b7b0'>
        <Header />

        <Box flex={1} mt={4} bg='white' borderTopLeftRadius={35} borderTopRightRadius={35} px={4}>
          <FlatList
            data={productData}
            contentContainerStyle={{ paddingVertical: 10, paddingHorizontal: 5, paddingBottom: 50 }}
            ItemSeparatorComponent={
              <Box h={4} w='100%' />
            }
            keyExtractor={(item) => item.id}
            pagingEnabled={true}
            showsVerticalScrollIndicator={false}
            refreshControl={
              <RefreshControl refreshing={loader} onRefresh={getAllProduct} />
            }

            renderItem={({ item }) => (

              <Box py={2} w='100%' bg='white' borderRadius={8} style={{ elevation: 8 }}  >
                <HStack w='100%' px={4} alignItems='center' >
                  <Box style={{ height: 50, width: 80 }} borderRadius={4} >
                    {
                      console.log("Image ===>", item?.services[0]?.image)
                    }
                    <Image alt='product_image' borderRadius={8} h='100%' w='100%' source={{ uri: 'https://www.infonlive.com/storage/' + item?.company_logo }} />
                  </Box>
                  <VStack ml={4} >
                    <Text fontSize={18} color='#000' fontWeight='bold' w={250} >{item?.name}</Text>
                    <HStack>
                      <Icon name="home" size={18} color='orange' />
                      <Text pr={2} fontSize={12} color='#000' w={200} >{item?.address}</Text>
                    </HStack>
                    <HStack>
                      <Icon name="location-on" size={18} color='orange' />
                      <Text pr={2} fontSize={12} color='#000'  >{item?.city_name},{item?.district_name}</Text>
                    </HStack>
                  </VStack>
                </HStack>

                <HStack w="100%" justifyContent='space-around' px={2} mt={2} >
                  {
                    item?.phone ?
                      <TouchableOpacity onPress={() => { Linking.openURL('tel:{item?.phone}') }} >
                        <Image alt='sample' h={7} w={7} source={{ uri: 'https://www.pngitem.com/pimgs/m/27-274866_phone-png-blue-phone-icon-png-blue-transparent.png' }} />
                      </TouchableOpacity>
                      :
                      null
                  }

                  {
                    item?.whatsapp_num ?
                      <TouchableOpacity onPress={() => Linking.openURL(`whatsapp://send?phone=${item?.whatsapp_num}&text=${'hi'}`)}>
                        <Image alt='sample' h={7} w={7} source={{ uri: 'https://png.pngtree.com/png-vector/20221018/ourmid/pngtree-whatsapp-mobile-software-icon-png-image_6315991.png' }} />
                      </TouchableOpacity>
                      :
                      null
                  }



                  {
                  item?.facebook_link ?
                      <TouchableOpacity  onPress={() => Linking.openURL('https://www.facebook.com/Amaze-beauty-parlour-1488888698024020/')} >
                        <Image alt='sample' h={7} w={7} source={{ uri: 'https://cdn4.iconfinder.com/data/icons/social-messaging-ui-color-shapes-2-free/128/social-facebook-circle-512.png' }} />
                      </TouchableOpacity>
                      :
                      null
                  }

                  {
                    item?.website_url ?
                      <TouchableOpacity onPress={() => Linking.openURL('http://amazebeautyparlour.com/')}>
                        <Image alt='sample' h={7} w={7} source={{ uri: 'https://www.nicepng.com/png/detail/18-187940_free-icons-png-small-website-icon.png' }} />
                      </TouchableOpacity>
                      :

                      null
                  }

                  {
                    item?.email ?
                      <TouchableOpacity onPress={()=>Linking.openURL('mailto:sminasherith@gmail.com')}>
                        <Image alt='sample' h={7} w={7} source={{ uri: 'data:image/png;base64,' }} />
                      </TouchableOpacity>
                      :
                      null
                  }


                  {
                    (item?.latitude && item?.longitude) ?
                    <TouchableOpacity  onPress={() => Linking.openURL('geo:{item?.latitude},{item?.longitude}')}>
                      <Image alt='sample' h={7} w={7} source={{ uri: 'https://img.lovepik.com/free-png/20210926/lovepik-map-location-icon-free-vector-illustration-png-image_401494181_wh1200.png' }} />
                      </TouchableOpacity>
                      :
                      null
                  }
                </HStack>
              </Box>
            )}
          />

        </Box>




      </Box>
    </NativeBaseProvider >
  )
}

export default Home

